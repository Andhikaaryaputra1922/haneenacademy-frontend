...
export default async function AdminQuizBuilderPage({ params }: { params: Promise<{ quizId: string }> }) {
    ...
    return (
      <main className="min-h-screen bg-[var(--base)] px-6 py-10">
        <div className="mx-auto max-w-3xl rounded-[40px] border border-[var(--border)] bg-[var(--surface)] p-7 md:p-10">
          ...
          <Link className="mt-6 inline-flex rounded-full bg-[var(--accent)] px-5 py-3 text-sm font-semibold text-[var(--text)]" href="/admin/quizzes">
            Kembali
          </Link>
        </div>
      </main>
